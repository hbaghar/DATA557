...
The independence assumptions are valid because each sample from the male and female group should be independent from each other.
...
- Equal variance $X$
  - As we can see in the above table, variances of the 3 groups are not very close. This assumption is getting violated.
- Large sample size or normal distribution of population $\checkmark{}$
  - There are at least 30 samples in each group population distribution of IQ tends to follow a normal distribution
...
for(row in 1:length(pairwise.df)){
  ...
  pairwise.df[row, "p.value"] <- t.test(x, y, paired = FALSE, var.equal = TRUE)$p.value
}
return(pairwise.df)
}
...
Based on this, we fail to reject the null hypothesis of all 3 tests. This in contradiction with our conclusions from Q8.
